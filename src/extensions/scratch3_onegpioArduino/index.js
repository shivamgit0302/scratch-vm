...
class Scratch3ArduinoOneGPIO {
    constructor(runtime) {
        ...
        this.digital_inputs = new Array(52).fill(0);
        this.analog_inputs = new Array(6).fill(0);
        this.pin_modes = new Array(52).fill(-1);
        this.wait_open = [];
    ...
    async digital_write(args) {
        console.log("digital_write called", args);
        if (!this.connected && !this.connection_pending) {
            await this.connect();
        }

        if (!this.connected) {
            console.log("Still not connected, queueing command.");
    ...
    async pwm_write(args) {
        ...
        if (!this.connected) {
            ...
            let pin = parseInt(args["PIN"], 10);
            let value = Math.round(255 * (parseInt(args["VALUE"], 10) / 100));
            if (this.pin_modes[pin] !== 3) {
                // PWM
                this.pin_modes[pin] = 3;
                this.sendSerial(
                    JSON.stringify({ command: "set_mode_pwm", pin })
                );
            }
            this.sendSerial(
    ...
    async tone_on(args) {
        ...
        if (!this.connected) {
            ...
            let pin = parseInt(args["PIN"], 10);
            let freq = parseInt(args["FREQ"], 10);
            let duration = Math.min(parseInt(args["DURATION"], 10), 5000);

            if (this.pin_modes[pin] !== 5) {
                // TONE
                this.pin_modes[pin] = 5;
                this.sendSerial(
                    JSON.stringify({ command: "set_mode_tone", pin })
                );
            }
            this.sendSerial(
                JSON.stringify({ command: "play_tone", pin, freq, duration })
            );
        }
    }
    ...
    async servo(args) {
        ...
        if (!this.connected) {
            ...
            if (this.pin_modes[pin] !== 4) {
                // SERVO
                this.pin_modes[pin] = 4;
                this.sendSerial(
                    JSON.stringify({ command: "set_mode_servo", pin })
                );
            }
            this.sendSerial(
                JSON.stringify({
    ...
    async analog_read(args) {
        ...
        if (!this.connected) {
            ...
            // Always send the command to get the latest value
            this.pin_modes[pin] = 7;
            await this.sendSerial(
                JSON.stringify({ command: "set_mode_analog_input", pin })
            );
    ...
    async digital_read(args) {
        ...
        if (!this.connected) {
            ...
            // Always send the command to get the latest value
            this.pin_modes[pin] = 1;
            await this.sendSerial(
                JSON.stringify({ command: "set_mode_digital_input", pin })
            );
    ...
    async sonar_read(args) {
        ...
        if (!this.connected) {
            ...
            // Always send the command to get the latest value
            this.pin_modes[trigger_pin] = 6;
            await this.sendSerial(JSON.stringify({ command: "set_mode_sonar", trigger_pin, echo_pin }));
    ...
    async connect() {
        ...
        if (this.connected || this.connection_pending) {
            console.log(
                "Already connected or connecting, ignoring connection attempt"
            );
            return;
        }
        ...
        try {
            ...
            // SET CONNECTED TO TRUE **BEFORE** await this.startReadLoop()
            this.connected = true;
            this.connect_attempt = true;
            this.alerted = false;

            // Now start the read loop (don't await it, just start it)
            this.startReadLoop();
            ...
            if (
                ...
                if (!this.alerted) {
                    this.alerted = true;
                    alert(
                        "Could not connect to Arduino. Make sure it's plugged in, the correct port is selected, and the custom sketch is uploaded."
                    );
                }
            }
        } finally {
    ...
    async sendSerial(data) {
        console.log("sendSerial called with:", data);
        if (!this.writer) {
    ...
    async handleDisconnect() {
        console.log("Disconnected!"); // Add this
