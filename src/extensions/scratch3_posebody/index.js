...
class Scratch3PoseBodyBlocks {
    ...
    getInfo () {
        return {
            ...
            name: formatMessage({
                ...
                description: 'Label for the body pose detection extension category'
            }),
        ...
        while (true) {
            ...
            if (frame) {
                ...
                try {
                    ...
                    if (this.bodyModel) {
                        this.poseState = await this.bodyModel.estimateSinglePose(frame, {
    ...
    goToPart (args, util) {
        ...
        const part = this.poseState.keypoints.find(point => point.part === args.PART);
        if (!part) {
        ...
        const scratchX = ((part.position.x / this.DIMENSIONS[0]) * stageWidth) - (stageWidth / 2);
        const scratchY = ((1 - (part.position.y / this.DIMENSIONS[1])) * stageHeight) - (stageHeight / 2);
    
        console.log('Converting to Scratch coordinates:', {scratchX, scratchY});
